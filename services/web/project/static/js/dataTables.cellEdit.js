...
jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function (settings) {
    ...
    jQuery.fn.extend({
        ...
        updateEditableCell: function (callingElement) {
            ...
            let columnName = $(callingElement).closest('table').find('th').eq(columnIndex).text();
            let rowIndex = $(callingElement).parents('tr').find("td:first").text();
            ...
            if (!newValue && ((settings.allowNulls) && settings.allowNulls != true)) {
                ...
                newValue
                && settings.onValidate && ((settings.validateColumns)
                && (settings.validateColumns.indexOf(columnIndex) > -1))
            ) {
                isValid = settings.onValidate(newValue);
                if (isValid) {
                    _update(newValue);
                } else {
                    _addValidationCss();
            ...
            function _addValidationCss() {
                ...
                loaderAlertMode('Такое значение уже существует');
            }
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
    let inputSetting, inputType, input, inputCss, confirmCss, cancelCss, startWrapperHtml = '', endWrapperHtml = '', listenToKeys = false;
    ...
    switch (inputType) {
        ...
        case "list-confirm": // List w/ confirm
            ...
            input.html = input.html + "</select>&nbsp;<a href='javascript:void(0);' class='" + confirmCss + "' onclick='$(this).updateEditableCell(this);'>Изменить</a> <a href='javascript:void(0);' class='" + cancelCss + "' onclick='$(this).cancelEditableCell(this)'>Отмена</a>" + endWrapperHtml;
            input.focus = false;
            break;
        ...
        case "textarea-confirm":
            input.html = startWrapperHtml + "<textarea id='ejbeatycelledit' class='" + inputCss + "'>"+oldValue+"</textarea><a href='javascript:void(0);' class='" + confirmCss + "' onclick='$(this).updateEditableCell(this)'>Изменить</a> <a href='javascript:void(0);' class='" + cancelCss + "' onclick='$(this).cancelEditableCell(this)'>Отмена</a>" + endWrapperHtml;
            break;
        case "number-confirm" :
